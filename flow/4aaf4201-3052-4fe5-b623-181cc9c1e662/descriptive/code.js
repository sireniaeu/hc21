...
var c;
try {
  var readResult = Fields["Notat - kliniker 2"].read();
  if (readResult) c = readResult.match(/([^\(]+)\s+(\(([^\)]+)\))?\s*(.*)/);
  else throw "Could not read clinician from field, trying window";
  if (!c) throw "Could not parse clinician from field, trying window";
  Log.warn('Kliniker', c);
  if (debug) Dialog.info('Kliniker', c);
} catch(err) {
  var title = Window.title();
  var t = title && title.match(/[^.,]+[.,]\s*([^,]+)[.,]\s*\((.*)\)/);
  if (!t) c = ["","","","",""];
  else c = ["",t[1],"","",t[2]];
  Log.warn('Windows name', c);
  if (debug) Dialog.info('Windows name', c);
}
