...
try {
  ...
  /* Spol gennem listen af patienter og grupper */
  ...
  { 
    ...
    /*if (debug2) Dialog.info('Status',patientsAndGroups.rows[i][kStatus], {});*/
    /* Hvis enkelt-patient fundet */
    if (patientsAndGroups.rows[i][kCpr] === patientid)
    {
      if (patientsAndGroups.rows[i][kTid] !== ptid) 
      {
        /* 
        Patient id fundet, CosmicTid er forskellig fra NoblyTid. 
        Kan f.eks. ske, hvis forbindelsen mellem Nobly Ankomst og Cosmic har været nede og brugerne har registrerert manuelt i Cosmic.
        Patienten kan have andre bookinger. Kun hvis alle patientens bookinger er markeret ankommet returneres "OK. Patient allerede registreret". 
        Ellers returneres: "Ikke-OK. Patient er booket til andet tidspunkt end kl. ”Nobly tid”.  */
        
        PtFoundDiffTime = true;
        if (patientsAndGroups.rows[i][kStatus] !== ankommet) PtArrived = false; 
      } else {
        /* Patient fundet. Tid korrekt */
    ...
    if (patientsAndGroups.rows[i][kCpr] === "" && patientFound === false)
    {
      ...
      try {   
        ...
        { 
          ...
          {            
            ...
            if (gtid !== ptid) 
            {
              ...
              /* Patient fundet - sæt flag for stop videre søgning */
              patientFound = true;
              j = group.rows.length;
              i = patientsAndGroups.rows.length;
              
            } /* Patient fundet med korrekt tid */
          } /* Slut på patient fundet i gruppe*/
        } /* Slut på spol gennem gruppe */ 
        
        /* Ryd op fra gruppe-siden og gå tilbage til besøgsliste */
        ...
        throw err;
      }  /* Slut på dobbeltklik gruppe */
    } /* Slut på gruppe i Besøgslisten*/
  
  } /* Slut på spol gennem Besøgslisten */
  
  /* Return fejlbesked, hvis patient ikke fundet eller patient fundet med anden tid og ikke registreret ankommet */
