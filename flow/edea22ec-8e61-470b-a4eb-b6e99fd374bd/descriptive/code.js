...
function cleanup(c) 
{
  ...
  if (timelog) Timer.log('PsykFlowTimer', 'Cleanup #1');
  try { Fields["Cave - Annullér knap"].click(); } catch (e) {}
  if (timelog) Timer.log('PsykFlowTimer', 'Cleanup #2');
  try { Fields["Cave - OK knap"].click(); } catch (e) {}
  if (timelog) Timer.log('PsykFlowTimer', 'Cleanup #3');
  try { Fields["Psy - Besøgsregistrering"].click(); } catch (e) {}
  if (timelog) Timer.log('PsykFlowTimer', 'Cleanup #4');
  Settings.CommandRetries = 6;
  try { var result = Fields["Psyk - gruppe tab"].select("P"); } catch (e) {}
  if (timelog) Timer.log('PsykFlowTimer', 'Cleanup #5');
  try { Fields["Ryd CPR"].click(); } catch (e) {}
  if (timelog) Timer.log('PsykFlowTimer', 'Cleanup #6');
  try { Fields["Psy - Ryd"].click(); } catch (e) {}
  if (timelog) Timer.log('PsykFlowTimer', 'Cleanup #7');
  try { Wait.forField(Fields["Psyk - besoegstabel"], 8); } catch (e) {} 
  if (timelog) Timer.log('PsykFlowTimer', 'Cleanup #8');
  Settings.CommandRetries = stdretry;
}
...
function gem() 
{
  ...
  try {      
    if (timelog) Timer.log('PsykFlowTimer', 'Start gem');
    ...
    Wait.forSeconds(1);
    if (ingroup) {
      Wait.forSeconds(9); /* It takes time for Cosmic to register the payment so returning from a group to fast prevents the payment registration */
    }
...
if (notify) Notification.show('Kaldt med', patientid + ' ' + ptid, pkliniker + ' ' + penhed, {timeout: 60});
//Log.warn('Psykiatri','Kliniker: ' + pkliniker + ' Enhed: ' + penhed);
...
try {
  ...
  { 
    ...
    if (patientsAndGroups.rows[i][kCpr] === "" && patientsAndGroups.rows[i][kKliniker] === pkliniker && patientFound === false)
    {
